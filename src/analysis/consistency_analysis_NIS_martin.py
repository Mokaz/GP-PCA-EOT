...
NAME = 'ekf' # Changed to a method that produces S and y
...
try:
    sim_data = np.load(f'data/results/simulation_data_{NAME}.pkl', allow_pickle=True)
except FileNotFoundError:
    print(f"Error: Simulation data file not found at 'data/results/simulation_data_{NAME}.pkl'")
...
try:
    num_timesteps = sim_data['num_frames']
except KeyError:
    num_timesteps = 300
    ...
    for i in range(num_simulations):
        ...
        for t in range(num_timesteps+1):
            try: 
                ...
                # NIS calculation
                if t != 0 and y_it is not None and S_it is not None:
                    try:
                        nis_t = (y_it.T @ np.linalg.inv(S_it) @ y_it).item()
                        nis.append(nis_t)
                    except np.linalg.LinAlgError:
                        nis.append(np.nan) # Handle singular S
                else:
                    nis_t = np.nan
                    nis.append(nis_t)
...
nis_upper = chi2.ppf(0.975, avg_z_dims)
nis_lower = chi2.ppf(0.025, avg_z_dims)
...
# Plot time-varying bounds
ax[1, 0].plot(time_steps, nis_upper, color='red', linestyle='--', alpha=0.6, linewidth=3)
ax[1, 0].plot(time_steps, nis_lower, color='blue', linestyle='--', alpha=0.6, linewidth=3)
ax[1, 0].plot([], [], color='k', label="Simulations")
ax[1, 0].plot([], [], 'r--', label="Upper Limit (97,5%)")
ax[1, 0].plot([], [], 'b--', label="Lower Limit (2,5%)")
ax[1, 0].set_title("NIS", fontsize=18)
...
print(f"Consistency plot saved to {PLOT_SAVE_PATH}")