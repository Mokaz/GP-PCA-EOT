...
def get_gp_tracker_config(lidar_pos, N_gp=20):
    ...
    return TrackerConfig(
        ...
        pos_north_std_dev=0.3, pos_east_std_dev=0.3, heading_std_dev=0.1, lidar_std_dev=0.05
    )
...
if __name__ == "__main__":
    ...
    length_scales = [np.pi / 6, np.pi / 4, np.pi / 2]
    forgetting_factors = [0.001, 0.01, 0.1]
    signal_vars = [0.5, 1.0, 2.0]

    for method in method_list:
        for ls in length_scales:
            for ff in forgetting_factors:
                for sv in signal_vars:
                    print(f"--- Setting up for method: {method}, ls={ls:.2f}, ff={ff:.4f}, sv={sv:.2f} ---")

                    if "gp" in method:
                        tracker_cfg = get_gp_tracker_config(lidar_base.lidar_position, N_gp)
                        tracker_cfg.gp_length_scale = ls
                        tracker_cfg.gp_forgetting_factor = ff
                        tracker_cfg.gp_signal_var = sv
                    else:
                        tracker_cfg = get_pca_tracker_config(lidar_base.lidar_position, N_pca)
                        # Skip loop for non-gp methods
                        if ls != length_scales[0] or ff != forgetting_factors[0] or sv != signal_vars[0]:
                            continue
                    ...
                    config.sim.name = f"study_{method}_{config.sim.seed}_ls_{ls:.2f}_ff_{ff:.4f}_sv_{sv:.2f}"
